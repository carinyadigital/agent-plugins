...
If working directory looks project-scoped and architecture may span repos, warn once that a project-scoped install cannot read files outside the project folder. Wait for confirmation.
...
When the user confirms target association, write `config/target.json` with `name`, `instance`, and `target`.
...
Location: `<instance-root>/config/.architecture-setup-resume.json` if instance exists; else `~/.claude/plugins/config/digital-agency/architecture/setup-resume.json`.
