...
Default: `.agency/backlog.md`. If the user names another path, use it.

## Arguments

- `--depth full` — detail for all roadmap phases (default: Now-phase epics only)
- `--stories` — lightweight story rows in this file; full Gherkin → `.agency/work/{epic}/tasks.md` via **tasks**

## Context

<artifacts>
[.agency/product.md, .agency/roadmap.md, .agency/architecture/solution.md]
</artifacts>
...
4. Every epic row: Epic ID, Title, Work path `.agency/work/{epic}/` (title or short title slug, max two words)
...
- [ ] Every epic has a unique `.agency/work/{epic}/` path; slug is kebab-case, ≤2 words, not Epic ID
...
Optional: run `bash scripts/check-epic-paths.sh .agency/backlog.md` from the backlog skill directory if the file exists.
