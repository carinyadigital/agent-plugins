...
| --------------- | ------- | ----- |
| `design.md` re-narrates module layout, NFRs, or cross-epic patterns | Boundary — cite `ARCHITECTURE.md §{N}` | `design` |
...
| Clash | Authoritative |
| ----- | ------------- |
| Architecture pattern (modules, NFRs, cross-epic) | `ARCHITECTURE.md`, then ADRs |
...
Legitimate restatement: a one-line pointer ("see `ARCHITECTURE.md` §5") is not
duplication. Flag only when the Solution Design restates enough that the two can drift.

## Citations

- [ ] `design.md` cites `ARCHITECTURE.md` by section where it relies on architecture.
