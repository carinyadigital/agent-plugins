...
Run the loop from [../SKILL.md](../SKILL.md), which is the canonical
definition. In short, per cycle:
...
5. Run the project's validation suite locally before every push. Push, wait for
   CI via the watch mechanism for the resolved tier — never a tight shell sleep
   loop.

## Hard limits

- Maximum **3** fix-push-check cycles without progress, then stop and report.
- The fix-vs-escalate boundary is defined in [../SKILL.md](../SKILL.md) —
