...
- `${CLAUDE_PLUGIN_ROOT}/references/engineering-conventions.md`
...
5. **Read** `~/.claude/plugins/config/digital-agency/engineering/CLAUDE.md` unless `--redo`.
...
If working directory looks project-scoped and engineering context may span repos, warn once that a project-scoped install cannot read files outside the project folder. Wait for confirmation.
...
1. **Implement** — `/engineering:implement <task-id>`
2. **Review** — `/engineering:code-review <branch>`
3. **MR** — `/engineering:merge-request`
...
1. **Deploy QA** — `/engineering:deploy-qa <branch>`
2. **Automated** — `/engineering:run-automated-suite`
3. **Exploratory** — `/engineering:exploratory-pass`

**Either:**

4. **Backlog** — `/product-management:tasks --product` (companion; requires product-management install)
5. **Refresh** — `/engineering:setup --redo` to redo engineering defaults only.
...
Location: `<instance-root>/config/.engineering-setup-resume.json` if instance exists; else `~/.claude/plugins/config/digital-agency/engineering/setup-resume.json`.
