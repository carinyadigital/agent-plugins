...
name: skills-qa
description: >
  Evaluate a skill against the Agency Skill Design Framework — thirteen design
...
- `~/.claude/plugins/config/digital-agency/plugin-management/CLAUDE.md` → engagement profile and installed skills list (provides context for evaluating whether the skill fits the user's team and workflow, and whether it duplicates something already installed)
...
Evaluates any skill against the Agency Skill Design Framework: **nine
...
- The skill's associated `CLAUDE.md` (template in the plugin directory, user config at `~/.claude/plugins/config/digital-agency/<plugin>/CLAUDE.md`) — if available, what engagement profile the skill reads and depends on
...
4. **Out-of-scope reads** — instructions to read paths outside the skill's own directory and `~/.claude/plugins/config/digital-agency/<plugin>/`. Flag specifically reads from: `~/.ssh/`, `~/.aws/`, `~/.config/gh/`, password managers, browser profiles, Mail, Messages, Slack files, or any path that could carry credentials.
...
When `/plugin-management:skills-qa` is invoked directly by the user (not as part of `/plugin-management:skills-qa`), cross-check the skill's source registry and publisher against any local allowlist the user provides (optional). This is passive information for the user — it does not gate the QA run, but it surfaces the install posture so a user running `/plugin-management:skills-qa` on a skill they want to install sees the allowlist status up front.
...
Missing frontmatter or required sections: **Some Concern.** Missing example AND guardrails in a agency skill: **Material Concern.**
...
When a stable convention surfaces during this run (thresholds, naming, tone, output format, or recurring corrections), **propose a profile update**: show the exact diff against `~/.claude/plugins/config/digital-agency/plugin-management/CLAUDE.md` (instance-wide facts go to `<instance-repo>/config/instance.json`), ask for confirmation, and write only on yes. Only practice `setup` skills auto-apply a full profile write.
