...
The `setup` skill copies or updates the user config at:
  ~/.claude/plugins/config/digital-agency/product-engineering/CLAUDE.md
...
*Written by `/product-engineering:setup` or initialized on first use.*

---

## Status

`template` — run `/product-engineering:setup` to fill this in.
...
*Re-check: `/product-engineering:setup --check-integrations`*
...
*Re-run: `/product-engineering:setup --redo`*
