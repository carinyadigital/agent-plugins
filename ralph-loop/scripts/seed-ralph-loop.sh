...
Required:
  ...
  --preset <name>               ad-hoc | custom | engineering-delivery
...
resolve_preset_file() {
  local name="$1"
  if [[ -f "$ASSETS/presets/$name.md" ]]; then
    printf '%s\n' "$ASSETS/presets/$name.md"
    return 0
  fi
  local d
  # Optional extra dirs (colon-separated) for local/out-of-tree presets.
...
PRESET_FILE="$(resolve_preset_file "$PRESET")" || die "unknown preset '$PRESET' (not in ralph-loop assets/presets)"
