...
`practice-setup` detects or creates `.digital-agency/target.json` for Golden Path 2
target binding.
...
| Skill | Purpose |
| ----- | ------- |
| **practice-setup** | Interview → write practice profile, target binding, stack defaults |
...
- **Instance profile** (optional) — `agency-hub:agency-setup` writes
  `config/instance.json`; practice-setup reads cadence and target hints without
  re-asking.
...
4. Re-run `/web-development:practice-setup --redo` to refresh engineering defaults.
