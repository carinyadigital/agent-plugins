...
ralph_evaluate() {
  ...
  if ! ralph_is_uint "$iteration"; then
    RALPH_REASON="active loop file corrupted (iteration: '$iteration'). Stopping. Re-seed with /web-development:ralph-loop-setup."
    ralph_clear_active "$base"
    return 0
  fi

  if ! ralph_is_uint "$max_iter"; then
    RALPH_REASON="active loop file corrupted (max_iterations: '$max_iter'). Stopping. Re-seed with /web-development:ralph-loop-setup."
    ralph_clear_active "$base"
    return 0
  fi
  ...
  if [[ $max_iter -gt 0 ]] && [[ $iteration -ge $max_iter ]]; then
    RALPH_REASON="max iterations ($max_iter) reached. Run /web-development:ralph-loop status to inspect progress."
    ralph_clear_active "$base"
    return 0
  fi
  ...
  if [[ -n "$state_file" && "$state_file" != "null" ]]; then
    ...
    if ralph_check_stall "$state_path" "$base/stall"; then
      RALPH_REASON="no progress in $RALPH_STALL_LIMIT consecutive iterations (state unchanged: $state_file). Stopping so a human can inspect. Run /web-development:ralph-loop status."
      ralph_clear_active "$base"
      return 0
    fi
  fi
