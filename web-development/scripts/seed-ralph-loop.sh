...
if [[ -f "$ACTIVE" && $FORCE -eq 0 ]]; then
  ...
  if [[ "$EXISTING_ITER" =~ ^[0-9]+$ ]] && [[ "$EXISTING_ITER" -gt 1 ]]; then
    ...
    printf '  Inspect it with /web-development:ralph-loop status, stop it with /web-development:ralph-loop cancel,\n' >&2
...
The loop is seeded but NOT running. Start it with: /web-development:ralph-loop start
EOF
